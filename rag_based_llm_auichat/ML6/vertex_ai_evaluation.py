...
def register_rag_as_vertex_model(
    ...
    try:
        ...
        model = aiplatform.Model.upload(
            ...
            serving_container_port=8080,  # Changed from ports (plural) to port (singular)
...
def run_vertex_evaluation(
    ...
    try:
        ...
        # First, run our custom evaluation (this doesn't depend on the model registration)
        evaluation_results = run_rag_evaluation(
            endpoint_url=endpoint_url,
            project_id=project_id,
            region=region
        ...
        return {
            "evaluation_results": evaluation_results,
            "vertex_ai_model": model_resource,
            "vertex_ai_job": job_resource
        }
        ...
        try:
            fallback_results = run_rag_evaluation(
                endpoint_url=endpoint_url,
                project_id=project_id,
                region=region
            )
            return {"evaluation_results": fallback_results}
